...
public class ConnectService extends Service {
	...
	private static final String LOGTAG = LogUtil
			.makeLogTag(ConnectService.class);
	...
	public void disconnect() {
		...
		taskSubmitter.submit(new Runnable() {
			public void run() {
				ConnectService.this.getXmppManager().disconnect();
			}
		});
	}
	...
	public class TaskSubmitter {
		...
		final ConnectService connectionService;

		public TaskSubmitter(ConnectService connectionService) {
		...
		public Future submit(Runnable task) {
			Future result = null;
			if (!connectionService.getExecutorService().isTerminated()
					&& !connectionService.getExecutorService().isShutdown()
					&& task != null) {
				result = connectionService.getExecutorService().submit(task);
			}
			return result;
		}
	...
	public class TaskTracker {
		...
		public TaskTracker(ConnectService connectionService) {
			this.connectionService = connectionService;
			this.count = 0;
		}

		public void increase() {
			synchronized (connectionService.getTaskTracker()) {
				connectionService.getTaskTracker().count++;
